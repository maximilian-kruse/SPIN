...
class MCMCSampler:
    ...
    def __init__(
        ...
        if not (
            isinstance(hlModel, model.SDEInferenceModel)
            and hlModel.grPosterior.mean is not None
        ):
            raise TypeError(
                "Need to provide hIPPYlib model for which"
        ...
        self._workGraph, self._samplingProblem = self._assemble_workgraph(
            self._inferenceModel
        )
        self._propDistr, self._proposal, self._MCMCKernel = self._init_kernel(
            samplerSettings, self._inferenceModel, self._samplingProblem
        )
        self._logger.print_ljust("Successful", end="\n\n")

    # -----------------------------------------------------------------------------------------------
    def run(
        self, runSettings: dict[str, Any], initialState: Optional[np.ndarray] = None
    ) -> Tuple[np.ndarray]:
        """Conducts sampling
        ...
        if initialState is None:
            if all(
                setting in runSettings.keys()
                for setting in ["init_variance", "init_seed"]
            ):
                initialState = self._init_solution(
                    runSettings["init_variance"], runSettings["init_seed"]
                )
            else:
                raise KeyError("Need to provide initial state or variance & seed.")
        elif not (
            isinstance(initialState, np.ndarray)
            and initialState.size == self._inferenceModel.funcSpaces[hl.PARAMETER].dim()
        ):
            raise TypeError(
                "Provided initial state needs to be numpy array with dimension"
        ...
        dataStructs = {
            "file_names": self._mcmcFileNames,
        ...
        meanData, varianceData, forwardData = self._logger.log_solution(
            paramsToInfer=self._inferenceModel.paramsToInfer,
    ...
    def evaluate_qoi(self, qoi: Optional["BaseQOI"] = None) -> np.ndarray:
        ...
        if qoi is None:
            ...
            raise TypeError(
                "Provided QOI needs to be valid object derived from BaseQOI."
            )
        else:
            qoi = qoi(self._inferenceModel.funcSpaces)

        qoiTrace = hm.cal_qoiTracer(self._inferenceModel.pdeProblem, qoi, self._samples)
        assert isinstance(qoiTrace, hl.QoiTracer), (
            "QOI trace is not a valid QoiTracer object"
        )
        self._logger.print_ljust("Successful")
        self._logger.print_arrays_to_file(
            self._qoiFile, ["QOI"], [qoiTrace.data], self._subDir
        )

        return qoiTrace.data

    # -----------------------------------------------------------------------------------------------
    def _assemble_workgraph(
        self, hlModel: model.SDEInferenceModel
    ) -> ms.SamplingProblem:
    ...
    def _init_kernel(
        ...
        samplerSettings: dict[str, Any],
        hlModel: model.SDEInferenceModel,
        samplingProblem: ms.SamplingProblem,
    ) -> Tuple:
        ...
        if samplerSettings["algorithm"] == "pCN":
            if "Beta" not in samplerSettings.keys():
                raise KeyError(
                    "Need to provide a value for setting 'Beta' when using pCN."
                )
            ...
            if "StepSize" not in samplerSettings.keys():
                raise KeyError(
                    "Need to provide a value for setting 'StepSize' when using MALA."
                )
            propFunc = ms.InfMALAProposal
            useZeroMean = True
        else:
            raise ValueError(
                "Unknown option for algorithm, supported are 'pCN' and 'MALA'."
            )

        if samplerSettings["use_gr_posterior"]:
            propDistr = hm.LAPosteriorGaussian(
                hlModel.grPosterior, use_zero_mean=useZeroMean
            )
        else:
    ...
    def _init_solution(self, variance: Union[int, float], seed: int) -> np.ndarray:
        ...
        posteriorSample = self._inferenceModel.inferenceModel.generate_vector(
            hl.PARAMETER
        )
        self._inferenceModel.grPosterior.sample(
            noise, priorSample, posteriorSample, add_mean=True
        )
...
class BaseQOI(ABC):
    ...
    def __init__(self, funcSpaces: list[fe.FunctionSpace]) -> None:
        ...
        if not (
            ...
            raise TypeError(
                "Need to provide list of three function spaces for forward variable,"
...
def postprocess_qoi(logger, qoiTrace: np.ndarray, maxLag: int) -> None:
    ...
    autoCorrFunc, confInt = sm.tsa.stattools.acf(
        qoiTrace, nlags=maxLag, alpha=0.1, fft=False
    )
