...
class TestData:
    ...
    interpValues = np.array(([0.75, 2.25],
                             [1.25, 3.75]))
...
def data_arr_vec() -> Tuple:
    ...
    npArray = np.linspace(TestData.lowerDomainBound, 
                          TestData.upperDomainBound, 
                          TestData.numMeshPoints+1)
...
def data_pyfunc_fefunc() -> Tuple:
    ...
    funcSpaceTensor1D = fe.TensorFunctionSpace(mesh, 
                                               'Lagrange', 
                                               TestData.femElemDegree, 
                                               shape = (1, 1), symmetry=True)
    funcSpaceTensor2D = fe.TensorFunctionSpace(mesh, 
                                               'Lagrange', 
                                               TestData.femElemDegree, 
                                               shape = (2, 2), symmetry=True)
    ...
    pyFuncData = [(dummyFuncScalar, funcSpaceScalar, gridPointsScalar),
...
def test_interpolation(data_arr_vec: Tuple) -> None:
    ...
    interpHandle = interpolation.InterpolationHandle(TestData.simTimes, 
                                                     TestData.obsTimes, 
                                                     TestData.obsPoints, 
                                                     funcSpace)
