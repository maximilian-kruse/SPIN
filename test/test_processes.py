...
class TestData:
    ...
    feSettings = {"num_mesh_points": 100,
                  "boundary_locations": [-5, 5],
                  "boundary_values": [0, 0],
                  "element_degrees": [1, 1]}

    transientSettings = {"start_time": 0.1,
                         "end_time": 5.1,
                         "time_step_size": 0.01,
...
#---------------------------------------------------------------------------------------------------
def test_drift_diffusion(process_setup: sp.BaseProcess, 
                         point_setup: Union[int, float, np.ndarray]) -> None:
...
#---------------------------------------------------------------------------------------------------
def test_stationary_distribution(process_setup: sp.BaseProcess,
                                 point_setup: Union[int, float, np.ndarray]) -> None:
    testProcess = process_setup
    testPoint = point_setup
...
#---------------------------------------------------------------------------------------------------
def test_mean_exit_time(process_setup: sp.BaseProcess,
                        point_setup: Union[int, float, np.ndarray]) -> None:
    testProcess = process_setup
    testPoint = point_setup
...
def test_transient_distribution(process_setup: sp.BaseProcess) -> None:
    testProcess = process_setup
    timePoints, pdfValues, funcSpace = \
        testProcess.compute_transient_distribution_fem(TestData.feSettings, TestData.transientSettings)
    spacePoints = funcSpace.tabulate_dof_coordinates().flatten()
    _test_transient_shape(spacePoints, timePoints, pdfValues)

#---------------------------------------------------------------------------------------------------
def test_data_stationary_fpe(process_setup: sp.BaseProcess,
                             point_setup: Union[int, float, np.ndarray]) -> None:
    testProcess = process_setup
    testPoint = point_setup
    perturbedValues, _ = testProcess.generate_data_stationary_fpe(testPoint, 
                                                                  TestData.dataStd, 
                                                                  TestData.dataSeed)
...
#---------------------------------------------------------------------------------------------------
def test_data_mean_exit_time(process_setup: sp.BaseProcess,
                             point_setup: Union[int, float, np.ndarray]) -> None:
    testProcess = process_setup
    testPoint = point_setup
    perturbedValues, _ = testProcess.generate_data_mean_exit_time(testPoint, 
                                                       TestData.dataStd, 
                                                       TestData.dataSeed,
                                                       TestData.metDomainBounds)

    _test_stationary_shape(testPoint, perturbedValues)

#---------------------------------------------------------------------------------------------------
def test_data_transient_fpe(process_setup: sp.BaseProcess,
                            point_setup: Union[int, float, np.ndarray],
                            time_setup: Union[int, float, np.ndarray]) -> None:
    testProcess = process_setup
    testPoint = point_setup
    testTime = time_setup
    perturbedValues, _ = testProcess.generate_data_transient_fpe(testPoint, 
                                                                 testTime,
                                                                 TestData.dataStd,
                                                                 TestData.dataSeed,
                                                                 TestData.feSettings, 
...
#---------------------------------------------------------------------------------------------------
def _test_stationary_shape(domainPoints: Union[int, float, np.ndarray], 
                           output: Union[int, float, np.ndarray]) -> None:
...
#---------------------------------------------------------------------------------------------------
def _test_transient_shape(domainPoints: Union[int, float, np.ndarray],
