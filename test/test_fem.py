...
def fem_space_setup() -> Tuple:
    ...
    funcSpaceVec = fe.VectorFunctionSpace(
        mesh, "Lagrange", TestData.femElemDegree, dim=TestData.domainDim
    )
    funcSpaceTensor = fe.TensorFunctionSpace(
        mesh, "Lagrange", TestData.femElemDegree, shape=2 * (TestData.domainDim,)
    )
...
def fem_var_setup(fem_space_setup: Tuple) -> Tuple:
    ...
    driftFunc = fe.interpolate(
        fe.Expression(("-x[0]",), degree=TestData.femElemDegree), funcSpaceVec
    )
    diffusionFunc = fe.interpolate(
        fe.Expression((("1.0",),), degree=TestData.femElemDegree), funcSpaceTensor
    )
...
# ---------------------------------------------------------------------------------------------------
def test_transient_assembly(
    fem_space_setup: Tuple,
    transient_problem_setup: Tuple,
    transient_matrix_setup: Tuple,
) -> None:
    _, funcSpace, *_ = fem_space_setup
    varForm, bcList = transient_problem_setup
    _, lhsMatrix, rhsVector = transient_matrix_setup

    testLhsMatrix, testRhsVector = femFunctions.assemble_transient(
        funcSpace, TestData.timeStepSize, varForm, bcList
    )
...
def test_transient_solve(
    ...
    solveSettings = {
        "time_step_size": TestData.timeStepSize,
        "init_sol": initSol,
...
def test_stationary_fem_problem() -> None:
    ...
    assert (
        isinstance(problem.funcSpaceVar, fe.FunctionSpace)
        and problem.funcSpaceVar.num_sub_spaces() == 0
    )
    assert (
        isinstance(problem.funcSpaceDrift, fe.FunctionSpace)
        and problem.funcSpaceDrift.num_sub_spaces() == domainDim
    )
    assert (
        isinstance(problem.funcSpaceDiffusion, fe.FunctionSpace)
        and problem.funcSpaceDiffusion.num_sub_spaces() == domainDim
    )
    assert (
        isinstance(problem.funcSpaceAll, fe.FunctionSpace)
        and problem.funcSpaceAll.num_sub_spaces()
        == int(0.5 * domainDim * (domainDim + 1)) + domainDim
    )

    assert isinstance(problem.boundCondsForward, list) and all(
        isinstance(bc, fe.DirichletBC) for bc in problem.boundCondsForward
    )
    assert isinstance(problem.boundCondAdjoint, list) and all(
        isinstance(bc, fe.DirichletBC) for bc in problem.boundCondAdjoint
    )


# ---------------------------------------------------------------------------------------------------
def test_transient_fem_problem() -> None:
    problem = femProblems.TransientFEMProblem(
        TestData.domainDim, TestData.solutionDim, TestData.feSettings, TestData.simTimes
    )
