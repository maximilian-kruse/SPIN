# hippykit: An extension of hIPPYlib at KIT

Customized Version of the hIPPYlib software library at the Karlsruhe Institute of Technology

Installation instructions for hIPPYlib apply. we recommend using `mamba` for the installation of `FEniCS`. In addition, hippykit depends on `sympy` and the
`fenics-enhancer` library, which is also developed at KIT.
`sympy` can easily be installed with `mamba`. For the `fenics-enhancer`, download the repository in the *hippylib@kit* Gitlab group and install the package locally.
