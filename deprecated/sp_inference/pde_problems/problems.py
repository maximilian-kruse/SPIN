...
class FEMProblem:
    ...
    def __init__(self, domainDim: int, solutionDim: int, feSettings: dict[str, Any]) -> None:
        ...
        self.mesh = self.set_up_mesh(
            feSettings["num_mesh_points"], feSettings["boundary_locations"]
        )
        (
            self.funcSpaceVar,
            self.funcSpaceDrift,
    ...
    def set_up_mesh(self, numMeshPoints: Union[int, list[int]], boundaryLocs: list) -> fe.Mesh:
        ...
        if self._domainDim == 1:
            ...
            if not isinstance(numMeshPoints, list):
                raise TypeError(
                    "1D problem requires list of two integers for number of mesh points."
                )
    ...
    def set_up_funcspaces(self, mesh: fe.Mesh, elemDegrees: list[int]) -> Tuple:
        ...
        if self._solutionDim == 1:
            ...
            funcSpaceVar = fe.VectorFunctionSpace(
                mesh, "Lagrange", elemDegrees[0], dim=self._solutionDim
            )
        funcSpaceDrift = fe.VectorFunctionSpace(
            mesh, "Lagrange", elemDegrees[1], dim=self._domainDim
        )
    ...
    # -----------------------------------------------------------------------------------------------
    def construct_boundary_functions(
        self,
...
class TransientFEMProblem(FEMProblem):
    ...
    def assemble(
        ...
        lhsMatrix, rhsConst = functions.assemble_transient(
            self.funcSpaceVar, self._dt, weakForm, self.boundCondsForward
        )
    ...
    ) -> Union[np.ndarray, hl.TimeDependentVector]:
