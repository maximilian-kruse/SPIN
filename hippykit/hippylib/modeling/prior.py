...
def BiLaplacianComputeCoefficients(
        sigma2: Union[numbers.Number, str],
        rho: Union[numbers.Number, str],
        Vh: dl.FunctionSpace) -> tuple[dl.Function]:
    """
    Compute the coefficients for the BiLaplacian operator.

    Parameters:
        sigma2 (Union[numbers.Number, str]): The value of sigma squared.
        rho (Union[numbers.Number, str]): The value of rho.
        Vh (dl.FunctionSpace): The function space.

    Returns:
        tuple[dl.Function]: A tuple containing the gamma and delta functions.
    """
    num_sub_spaces = Vh.num_sub_spaces()
    dim = Vh.mesh().geometry().dim()
    if num_sub_spaces > 1:
        raise ValueError(f"Given function space has {num_sub_spaces} components, "
                         f"but only scalar coefficients have been provided.")

    sigma2 = fee.convert_to_np_array(sigma2, Vh)
    rho = fee.convert_to_np_array(rho, Vh)
    gamma, delta = _convert_coefficients(sigma2, rho, dim)
    gamma_func = fee.create_fe_function_from_np_array(gamma, Vh)
    delta_func = fee.create_fe_function_from_np_array(delta, Vh)
    
    return gamma_func, delta_func

@multimethod
def BiLaplacianComputeCoefficients(sigma2: Iterable[Union[numbers.Number, str]],
                                   rho: Iterable[Union[numbers.Number, str]],
                                   Vh: dl.FunctionSpace) -> tuple[dl.Function]:
    """
    Computes the coefficients for the bi-Laplacian computation.

    Args:
        sigma2 (Iterable[Union[numbers.Number, str]]): A collection of values representing
                                                       the sigma2 coefficients.
        rho (Iterable[Union[numbers.Number, str]]): A collection of values representing
                                                    sthe rho coefficients.
        Vh (dl.FunctionSpace): The function space.
    ...
    Raises:
        ValueError: If the number of sub spaces in Vh is greater than 2.
        ValueError: If the length of sigma2 and rho are not equal.
        ValueError: If the length of sigma2 is not equal to the number of sub spaces.
    """
    num_sub_spaces = Vh.num_sub_spaces()
    dim = Vh.mesh().geometry().dim()
    if num_sub_spaces > 2:
        raise ValueError(f"Given function space has {num_sub_spaces} components, "
                         f"but needs at least two to call this method")
    if not len(sigma2) == len(rho):
        raise ValueError(f"sigma2 ({len(sigma2)}) and rho ({len(rho)}) "
                         f"need to have the same number of components.")
    if not len(sigma2) == num_sub_spaces:
        raise ValueError(f"Coefficients  need to have the same number of components "
                            f"({len(sigma2)}) as the given function space ({num_sub_spaces}).")
    gamma = []
    delta = []
    for i, components in enumerate(zip(sigma2, rho)):
        components = list(components)
        sub_space = Vh.extract_sub_space([i]).collapse()
        for j, component in enumerate(components):
                components[j] = fee.convert_to_np_array(component, sub_space)
        gamma_component, delta_component = _convert_coefficients(*components,
                                                                 dim)
        gamma.append(gamma_component)
        delta.append(delta_component)

    gamma = fee.convert_np_array_np2fe(gamma, Vh)
    delta = fee.convert_np_array_np2fe(delta, Vh)
    gamma_func = fee.create_fe_function_from_np_array(gamma, Vh)
    delta_func = fee.create_fe_function_from_np_array(delta, Vh)
    
    return gamma_func, delta_func

def _convert_coefficients(sigma2: np.ndarray,
                          rho: np.ndarray,
                          ndim: int) -> tuple[np.ndarray]:
    """
    Calculate the gamma and delta coefficients based on the given parameters.

    Parameters:
        sigma2 (np.ndarray): Array of variance values.
        rho (np.ndarray): Array of rho values.
        ndim (int): Number of dimensions. Can only take values 1, 2, or 3.

    Returns:
        gamma (np.ndarray): Array of gamma coefficients.
        delta (np.ndarray): Array of delta coefficients.
    """
...
def BiLaplacianPrior(Vh, gamma, delta, Theta = None, mean=None,
                     rel_tol=1e-12, max_iter=1000, robin_bc=False, robin_const=1.42):
    """
    ...
    - :code:`robin_const`:     (inverse) constant for the Robin boundary condition
    """
    ...
    def _compute_component_form(trial_c, test_c, gamma_c, delta_c, robin_bc_c, robin_const_c):
        if Theta is None:
            varfL = gamma_c*ufl.inner(ufl.grad(trial_c), ufl.grad(test_c))*ufl.dx
        else:
            varfL = gamma_c*ufl.inner( Theta*ufl.grad(trial_c), ufl.grad(test_c))*ufl.dx
        varfM = delta_c*ufl.inner(trial_c, test_c)*ufl.dx
        
        if robin_bc_c:
            robin_coeff = gamma_c*ufl.sqrt(delta_c/gamma_c)/dl.Constant(robin_const_c)
        else:
            robin_coeff = dl.Constant(0.)    
        varf_robin = robin_coeff*ufl.inner(trial_c, test_c)*ufl.ds
        
        return varfL + varfM + varf_robin

    def sqrt_precision_varf_handler(trial, test):
        if Vh.num_sub_spaces() == 0:
            return _compute_component_form(trial, test, gamma, delta, robin_bc, robin_const)
        else:
            varf = []
            for i in (0, 1):
                varf.append(_compute_component_form(trial[i],
                                                    test[i],
                                                    gamma[i],
                                                    delta[i],
                                                    robin_bc[i],
