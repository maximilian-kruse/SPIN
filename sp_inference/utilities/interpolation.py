...
class InterpolationHandle:
    ...
    def __init__(self,
                 ...
                 funcSpace: fe.FunctionSpace) -> None:
        """Constructor
        ...
        if not all(isinstance(timeStruct, (int, float, np.ndarray))
    ...
    def interpolate_in_time(self, simVec: hl.TimeDependentVector) -> hl.TimeDependentVector:
        ...
        if not np.array_equal(self._simTimes, simVec.times):
            raise ValueError("Simulation times and times for construction of time interpolation "
                             "need to be equal.")
    ...
    def project_in_space(self, interpVec: hl.TimeDependentVector) -> hl.TimeDependentVector:
        ...
        if not np.array_equal(self._obsTimes, interpVec.times):
            raise ValueError("Observation times and times for construction of space projection "
                             "need to be equal.")
