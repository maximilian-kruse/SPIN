...
class MatplotlibFigure:
    ...
    def make_axis_title(self) -> None:
        if not self.title_is_set:
            title = ""
            if prefix := self.title["prefix"]:
                title += f"{prefix}, "
            if component := self.title["component"]:
                title += f"component {component}, "
            if timestamp := self.title["timestamp"]:
                title += f"time = {timestamp}, "
            if suffix := self.title["suffix"]:
                title += f"{suffix}"
    ...
    def prune_and_save(self) -> None:
        if not self.axis.has_data():
            ...
            file_name = ""
            if prefix := self.file_name["prefix"]:
                file_name += f"{prefix}_"
            if component := self.file_name["component"]:
                file_name += f"c{component}_"
            if timestamp := self.file_name["timestamp"]:
                file_name += f"t{timestamp}_"
            if suffix := self.file_name["suffix"]:
                file_name += f"{suffix}"
...
class Pyplot2DFigure(MatplotlibFigure):
    ...
    def make_axis_legend(
        self, label: str, x_lims: Iterable[int], y_lims: Iterable[int]
    ) -> None:
        ...
        for i in range(self.num_components):
            ...
            for j in range(self.num_timestamps):
                if not len(self.figure_handles[i][j]) == self.num_sub_inds:
                    raise ValueError(
                        f"Number of figure handles ({len(self.figure_handles[i][j])}) "
...
class FigureGenerator:
    ...
    def __init__(self, inference_data_list: Iterable[dt.InferenceData]) -> None:
        self._data_list = inference_data_list
        self._num_data_structs, self._num_components = check_inference_data_list(
            inference_data_list
        )
    ...
    def build_figure_collection(self) -> Iterable:
        ...
        figure_collection = FigureCollection(
...
class Visualizer:
    ...
    _time_inds_to_plot = staticmethod(
        lambda time_array: (0, int(time_array.size / 2), time_array.size - 1)
    )
    ...
    # ============================================ API ==============================================
    def plot_inference_data(
        self, *inference_data_list: Iterable[dt.InferenceData], path_name: str
    ) -> None:
    ...
    def _plot_component(
        ...
        self._plot_component_snapshot(
            (list_ind, component, 0), domain, values, label, is_point_data
        )
    ...
    def _plot_component(
        ...
        for i, time_ind in enumerate(time_inds_to_plot):
            ...
            self._plot_component_snapshot(
                (list_ind, component, i), domain, values, label, is_point_data
            )

    # -----------------------------------------------------------------------------------------------
    @multimethod
    def _plot_component_interval(
        self,
        indices: Iterable[int],
        field: dt.StationaryField,
        ...
        self._plot_component_interval_snapshot(
            (list_ind, component, 0),
            domain,
            mean_values,
            var_values,
            label,
    ...
    def _plot_component_snapshot(
        ...
        if not len(indices) == 3:
            raise ValueError(
                "Method needs three indices (param_ind, component, timestamp)"
            )
    ...
    # -----------------------------------------------------------------------------------------------
    @multimethod
    def _plot_component_interval_snapshot(
        self,
        indices,
        domain: dt.Grid1D,
        ...
        if is_point_data:
            PlottingFunctions.plot_1D_errorbars(
                handle, x_values, y_mean_values, y_var_values, self._interval_scale
            )
        else:
            PlottingFunctions.plot_1D_interval(
                handle, x_values, y_mean_values, y_var_values, self._interval_scale
            )
    ...
    def _plot_component_snapshot(
        ...
        z_values = values
        handle = self._figure_collection.figure_handles[component][timestamp][
            2 * list_ind
        ]
        ...
        if is_point_data:
            PlottingFunctions.plot_2D_grid_points(
                handle, x_values, y_values, z_values, label
            )
        else:
            PlottingFunctions.plot_2D_grid_contour(
                handle, x_values, y_values, z_values, label
            )
...
class PlottingFunctions:
    ...
    @staticmethod
    def plot_1D_line(
        handle: Pyplot1DFigure, x_values: np.ndarray, y_values: np.ndarray, label: str
    ):
    ...
    def plot_2D_grid_points(
        ...
        handle.axis.set_xlabel("x")
        handle.axis.set_ylabel("y")
        plot2D = handle.axis.scatter(
            stacked_x_values, stacked_y_values, c=stacked_z_values, cmap=colormap
        )
        handle.fig.colorbar(plot2D)
        handle.make_axis_legend(
            label,
...
def plot_hessian_data(eigenvalues: np.ndarray, path_name: str) -> None:
    ...
    if not eigenvalues.ndim == 1:
        raise ValueError(
            f"Hessian eigenvalues must be 1D array, "
...
def plot_qoi_data(
    ...
    if not (len(acf_interval) == 2 and acf_interval[0].size == acf_interval[1].size):
        raise ValueError(
            "ACF interval needs to be provided as 2 arrays of the same size"
        )
    ...
    ax.fill_between(
        acf_inds, acf_interval[0], acf_interval[1], color="royalblue", alpha=0.25
    )
    ax.axhline(y=0)
